...
if ($_SERVER["REQUEST_METHOD"] == "POST" && Form::testToken('license-agreement-form') === true) {
    // the signature cannot be required because the formvalidation plugin doesn't accept required hidden fields, so we have to validate it manually and create a custom error message.
    if (empty($_POST['user-signature'])) {
        ...
        $validator->recaptcha('6LeNWaQUAAAAAOnei_86FAp7aRZCOhNwK3e2o2x2', 'Captcha Error')->validate('g-recaptcha-response');

        // check for errors
        if ($validator->hasErrors()) {
            $_SESSION['errors']['license-agreement-form'] = $validator->getAllErrors();
        } else {
            $dataUri = $_POST['user-signature'];
            $encodedImage = explode(',', $dataUri)[1];
            $decodedImage = base64_decode($encodedImage);

            // remove comments to save the signature on your server
            // file_put_contents('signature.png', $decodedImage);
...
$form = new Form('license-agreement-form', 'vertical', 'novalidate', 'tailwind');
$form->setMode('development');
$form->startFieldset('Please read the terms and conditions of the license carefully', '', 'class=text-2xl dark:text-white my-6');

if ($hasSignatureError) {
    $signatureErrorMsg = Form::buildAlert('Please add your signature to confirm your agreement', 'tailwind', 'danger');
    $form->addHtml($signatureErrorMsg);
}
...
$form->addTextarea('license', $licenseText, 'Terms & conditions', 'rows=7, style=font-size:11px, readonly');
$form->addInput('hidden', 'user-signature', '', 'Sign to confirm your agreement', 'data-signature-pad=true, data-background-color=#F7F7F7, data-pen-color=#333, data-width=100%, data-clear-button=true, data-clear-button-class=text-white bg-amber-500 hover:bg-amber-600 focus:ring-4 focus:ring-amber-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-4 mb-8 dark:focus:ring-amber-900, data-clear-button-text=clear');
$form->centerContent();
$form->addRecaptchaV3('6LeNWaQUAAAAAGO_c1ORq2wla-PEFlJruMzyH5L6');
$form->addBtn('submit', 'submit-btn', 1, 'Agree <i class="fas fa-check ml-4" aria-hidden="true"></i>', 'class=text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-4 mb-8 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800, data-ladda-button=true, data-style=zoom-in');
$form->endFieldset();
...
<head>
    ...
    <title>Tailwind License agreement Form with digital signature - How to create PHP forms easily</title>
    <meta name="description" content="Tailwind Form Generator - how to create a License agreement Form with digital signature with Php Form Builder">
    <link rel="canonical" href="https://www.phpformbuilder.pro/templates/tailwind-forms/license-agreement.php" />
...
<body>
    ...
    <h1 class="text-center">Php Form Builder - Tailwind License agreement Form<br><small>with electronic signature</small></h1>
