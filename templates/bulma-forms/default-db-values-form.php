...
if ($_SERVER["REQUEST_METHOD"] == "POST" && Form::testToken('db-form') === true) {
    ...
    if ($validator->hasErrors()) {
        ...
        if ($_SERVER['HTTP_HOST'] != 'www.phpformbuilder.pro' && !$db->update('products', $update, $where)) {
            ...
            $msg = Form::buildAlert('Database updated successfully!', 'bulma', 'success');
        }
    }
}
...
$form = new Form('db-form', 'horizontal', 'novalidate', 'bulma');
$form->setMode('development');

$form->addInput('hidden', 'product_id');
$form->startFieldset('Update Product', '', 'class=pl-3 is-size-4 has-text-centered mb-4');
...
$form->setCols(0, 12);
$form->centerContent();
$form->addBtn('button', 'cancel', 0, 'Cancel', 'class=button is-warning mt-5', 'btn-group');
$form->addBtn('submit', 'submit-btn', 1, 'Submit', 'class=button is-primary mt-5, data-ladda-button=true, data-style=zoom-in', 'btn-group');
...
<head>
    ...
    <title>Bulma Form - Populate form with values from database</title>
    <meta name="description" content="Bulma Form Generator - how to retrieve default fields values from database with Php Form Builder">
    <link rel="canonical" href="https://www.phpformbuilder.pro/templates/bulma-forms/default-db-values-form.php" />
...
<body>
    ...
    <h1 class="has-text-centered is-size-1 mb-6">Php Form Builder - Form using Database values<br><small>Populate form with values from database</small></h1>
